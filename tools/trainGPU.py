...
def train(cfg):
    ...
    if cfg.MODEL.IF_WITH_CENTER == 'no':
        ...
        if cfg.MODEL.PRETRAIN_CHOICE == 'self':
            ...
            scheduler = WarmupMultiStepLR(optimizer, cfg.SOLVER.STyixEPS, cfg.SOLVER.GAMMA, cfg.SOLVER.WARMUP_FACTOR,
...
def main():
    ...
    # # cudnn.benchmark = False
    # argsXtt=["INPUT.PADDING",listPadding[np.argmax(resPadding)]]
    # cfg.merge_from_list(argsXtt)
    # f=open(os.path.join(cfg.OUTPUT_DIR,"choosePW.txt"),"w")
    # listWarm=[0.005,0.007,0.02,0.05]
    # resWarm=[]
    ...
    #     resPadding.append(tmp)
    #     cudnn.benchmark = False
    # f.write("best padding: "+str(listPadding[np.argmax(resPadding)])+";best warmup_factor: "+str(listWarm[np.argmax(resWarm)])+";map: "+str(max(resWarm)))
    # f.close()
