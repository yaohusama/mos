...
from modeling import build_model,build_modelQ
...
def train(cfg):
    ...
    # prepare model
    model = build_modelQ(cfg, num_classes,camera_num=0,view_num=0)
    ...
    if cfg.MODEL.IF_WITH_CENTER == 'no':
        ...
        loss_func = make_loss1(cfg, num_classes)     # modified by gu
...
def main():
    ...
    listPadding=[0.00008,0.00011,0.00013] #[0.0003,0.00025,0.00023,0.0002,0.00018,0.00015,0.00013,0.0001]
    resPadding=[]
    ...
    # cudnn.benchmark = False
    argsXtt=["SOLVER.BASE_LR",listPadding[np.argmax(resPadding)]]
    cfg.merge_from_list(argsXtt)
    f=open(os.path.join(cfg.OUTPUT_DIR,"choosePW.txt"),"w")
    listWarm=[0.0003,0.0005,0.0007]#[0.005,0.007,0.02,0.05]
    resWarm=[]
    for item in listWarm:
        argsXtt=["SOLVER.WEIGHT_DECAY",item]
        cfg.merge_from_list(argsXtt)
        cfg.freeze()
        # output_dir = cfg.OUTPUT_DIR
        # if output_dir and not os.path.exists(output_dir):
        ...
        resWarm.append(tmp)
        cudnn.benchmark = False
    f.write("best lr: "+str(listPadding[np.argmax(resPadding)])+";best weight decay: "+str(listWarm[np.argmax(resWarm)])+";map: "+str(max(resWarm)))
    f.close()
