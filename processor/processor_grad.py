...
def do_train(cfg,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader):
            ...
            image = img.to(device)
            img = nn.Parameter(image, requires_grad=True)
            ...
            mask = grad.cpu().detach().numpy()[0] #3,256,128
            mask = np.sum(mask, axis=0)  # [H,W]
            mask = np.maximum(mask,0) #256,128, ReLu
            mask -= np.min(mask)
            mask /= np.max(mask)

            mask = cv2.applyColorMap(np.uint8(255 * mask), cv2.COLORMAP_JET)
            mask = np.float32(mask) / 255
            mask = mask[..., ::-1]  # gbr to rgb
            plt.matshow(mask)
            plt.show()
