...
class MSMT17(BaseImageDataset):
    ...
    Dataset statistics:
    # identities: 4101
    # images: 32621 (trainer) + 11659 (query) + 82161 (gallery)
    # cameras: 15
    """
    dataset_dir = 'MSMT17_V1' #_V1

    def __init__(self, root='', verbose=True, pid_begin=0, **kwargs):
        super(MSMT17, self).__init__()
        self.pid_begin = pid_begin
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.train_dir = osp.join(self.dataset_dir, 'train')     #trainer
        self.test_dir = osp.join(self.dataset_dir, 'test') #test
